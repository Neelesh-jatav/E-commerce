...
export const placeNewOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  let total_price = 0, values = [], placeholders = [];
  items.forEach((item, idx) => {
    const product = products.find(p => p.id === item.product.id);
    if (!product) return next(new ErrorHandler(`Product not found for ID: ${item.product.id}`, 404));
    if (item.quantity > product.stock) return next(new ErrorHandler(`Only ${product.stock} units available for ${product.name}`, 400));
  ...
  const tax_price = 0.18, shipping_price = total_price >= 50 ? 0 : 2;
  ...
  res.status(200).json({ success: true, message: "Order placed successfully. Please proceed to payment.", 
...
export const fetchSingleOrder = catchAsyncErrors(async (req, res) => {
  ...
  const result = await database.query(`
    SELECT o.*, COALESCE(json_agg(json_build_object(
      ...
      'quantity', oi.quantity,'price', oi.price)) FILTER (WHERE oi.id IS NOT NULL), '[]') AS order_items,
      json_build_object('full_name', s.full_name,'state', s.state,'city', s.city,'country', s.country,
      'address', s.address,'pincode', s.pincode,'phone', s.phone) AS shipping_info
    FROM orders o
    LEFT JOIN order_items oi ON o.id = oi.order_id
    LEFT JOIN shipping_info s ON o.id = s.order_id
    WHERE o.id = $1 GROUP BY o.id, s.id;`, [orderId]);
